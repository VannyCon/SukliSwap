...
require_once('../services/TrCoinRequestService.php');
...
$svc = new TrCoinRequestService();

// Public reads: list requests for an offer
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
	$action = $_GET['action'] ?? '';
    switch ($action) {
        case 'listByPostRequest':
            $postRequestId = intval($_GET['post_request_id'] ?? 0);
            $result = $svc->listByPostRequest($postRequestId, 0);
...
$middleware->requireAuth(function() {
	...
	switch ($action) {
		...
		case 'accept':
			...
			// Debug logging
			error_log("Accept request - ID: $id, UserID: $userId, ScheduledTime: $scheduledMeetingTime");
